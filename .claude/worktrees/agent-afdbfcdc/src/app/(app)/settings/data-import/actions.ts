  ...
  try {
    ...
    void (async () => {
      ...
      try {
        ...
        // Step 2: Deduplicate locations by hotel name (D-03)
        ...
        >();
        for (const item of allItems) {
          const hotelName = extractLocationName(item);
          if (hotelName && !uniqueLocations.has(hotelName)) {
            uniqueLocations.set(hotelName, mapMondayItemToLocation(item, mappings).locationData);
          }
        }
        ...
        for (const item of allItems) {
          ...
          kioskItemMeta.push({ hotelName: extractLocationName(item), item });
