...
export const kioskColumns: ColumnDef<KioskListItem>[] = [
  ...
  // 2. Kiosk ID — non-editable (primary identifier link), has header filter
  {
    accessorKey: "kioskId",
    enableSorting: true,
  ...
  // 4. Venue Name — non-editable (managed via assignment flow), has header filter
  {
    ...
    cell: ({ getValue }) => {
      ...
      return val ?? <span className="text-wk-mid-grey">Unassigned</span>;
    },
  },
  ...
  // 9. Hardware Model (hidden by default) — editable
  {
  ...
  // 10. Software Version (hidden by default) — editable
  {
  ...
  // 11. Maintenance Fee (hidden by default) — non-editable (currency, leave for detail)
  {
  ...
  // 12. Free Trial Status (hidden by default) — non-editable (boolean)
  {
  ...
  // 13. Created At (hidden by default) — non-editable (system field)
  {
...
// Default hidden columns (only visible ones are shown by default)
