...
export async function saveView(name: string, config: LocationViewConfig, viewType: string = "table") {
...
export async function updateView(viewId: string, name: string, config: LocationViewConfig) {
  const parsed = saveViewSchema.parse({ name });
  const session = await getSessionOrThrow();
