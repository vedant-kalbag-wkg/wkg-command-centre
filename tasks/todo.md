...
- [x] **5.7** D8 + D5 — Address-data-quality flagged via `merge_proposals.decision='address_fix'` for 4 clusters (2: NH Eurobuilding/Spark by Hilton; 3: Melia Barajas/IBIS Madrid; 10: Maldron Shoreditch/Clayton London Wall; 15: Sheraton Heathrow/Hotel Zentral Castellana Norte). All four are UK-default-address import-bug shape (D5). **Phase 5.7 follow-up:** real-address lookup + correction via Monday re-pull or hand-edit; notes captured in each proposal row.
...
- [x] **6.7** Lat/lng population: geocoded `locations.address` via Google Maps Geocoding API; admin UI at `/settings/geocoding` with dry-run preview, skip-existing default, force-rerun checkbox, and per-row audit log (`metadata.script='scripts/geocode-locations.ts'`, `provider='google'`). Code-complete on `gsd/phase-06-…` branch; the staging-DB apply against the real Google Maps API is gated on `GOOGLE_MAPS_API_KEY` being added to Vercel env vars and runs as the manual UAT step (Task 4 of Phase 6 plan 06-06).
