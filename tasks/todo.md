...
- [ ] **6.2** Threshold magic numbers (±10% plateau, 70/40 heat-map, 80/50/20 outlet tiers) → settings table. (P2 — **deferred to follow-up PR**: schema design + migration + admin UI for editing; not orthogonal to 6.6 so they should land together in a "thresholds-as-settings" feature.)
...
- [ ] **6.6** Threshold editor: persist to URL params + write audit log on change. (P2 — **deferred to follow-up PR**: orthogonal to 6.2 and benefits from landing alongside the threshold-settings table.)
