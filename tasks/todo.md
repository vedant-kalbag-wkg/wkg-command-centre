...
- [ ] **6.7** Lat/lng population: geocode `locations.address` via Google Maps (or OpenStreetMap Nominatim). (P2 — **deferred to follow-up PR**: external API + rate-limit + 392 outlets means a separate small PR with credentials + dry-run mode is the right shape.)
