...
- [~] **5.2** Backfill historical install dates from chosen source per **D4**. — `scripts/backfill-kiosk-install-dates.ts` written; dry-run against prod confirms plan: 362 → `live_date`, 0 → `MIN(transaction_date)` (no sales-only locations have active kiosks), 7 untouched (per D4 — neither liveDate nor sales), 11 archived skipped. **`--apply` deferred to post-merge runbook.**
...
- [ ] **5.4** Re-validate Maturity dashboard: install cohorts span multiple months; ramp curve shows real growth shape. (P0 — verification gate, post-apply)
